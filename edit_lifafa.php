...
if (!file_exists($lifafaFile)) {
    die("❌ Lifafa not found!");
}

$lifafas = json_decode(file_get_contents($lifafaFile), true);
$lifafa = $lifafas[$lifafaId] ?? null;

if (!$lifafa || $lifafa['created_by'] != $number) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $lifafas[$lifafaId]['title'] = $title;
    $lifafas[$lifafaId]['comment'] = $comment;
    $lifafas[$lifafaId]['redirect'] = $redirect;
    $lifafas[$lifafaId]['access_code'] = $accessCode;

    file_put_contents($lifafaFile, json_encode($lifafas, JSON_PRETTY_PRINT));
    echo "<script>alert('✅ Lifafa updated successfully!'); location.href='my-lifafa.php';</script>";
    exit;
}
?>
<!DOCTYPE html>
 ...
 Edit Lifafa
  ...
  <div class="group11">
    <input 
      ...
      type="text" 
      name="title"
      value="<?= htmlspecialchars($lifafa['title']) ?>" 
...
<svg class="icon11" width="24" height="24" viewBox="0 0 120 60" xmlns="http://www.w3.org/2000/svg">
  ...
  <div class="group11">
    <input 
      ...
      type="text" 
      name="comment" 
      value="<?= htmlspecialchars($lifafa['comment']) ?>"
      placeholder="Enter Giveaway Comment" 
      required
    >
<svg class="icon11" width="24" height="24" viewBox="0 0 120 60" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  <!-- Per User Amount -->
    ...
    <input 
      class="input11" 
      type="url" 
      value="<?= htmlspecialchars($lifafa['redirect']) ?>"
      name="redirect" 
...
<svg class="icon11" width="24" height="24" viewBox="0 0 120 60" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Per User Amount -->
    ...
    <input 
      class="input11" 
      type="text" 
      value="<?= htmlspecialchars($lifafa['access_code']) ?>"
      name="access_code" 
