...
<div class="card" style="display: flex; justify-content: center; align-items: center; padding: 20px;">
  <h2 style="color: blue; display: flex; align-items: center; justify-content: center; margin: 0;">
    <i class="fa-solid fa-cart-shopping" style="color: green; margin-right: 8px;"></i>
    PCL ~ Shopping Mall
  </h2>
</div>
...
<?php if (empty($products)): ?>
  ...
  <div class="custom-alert warning">⚠️ No products available.</div>
  </div>
<?php else: ?>
  <?php foreach ($products as $product): ?>
    <div class="product">
      <img src="<?= $product['path'] . '/' . $product['image'] ?>" alt="<?= htmlspecialchars($product['title']) ?>">
      <div class="product-details">
          
        <div class="title2"><?= htmlspecialchars($product['title']) ?></div>
        
        <div class="desc"><?= htmlspecialchars($product['description']) ?></div>
        
        <div class="price2">₹ <?= number_format($product['price'], 2) ?></div>
        <br>
        <a href="view_product.php?product=<?= urlencode($product['filename']) ?>" 
           class="btn-submit downloadBtn" 
           data-href="view_product.php?product=<?= urlencode($product['filename']) ?>">
