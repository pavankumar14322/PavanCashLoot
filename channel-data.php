...
<?php
$dataFile = __DIR__ . "/users/$number/channels/$number.json";
...
$bot_username = $adminData['bot_username'];
$admin_chat_id = $adminData['admin_chat_id'];

function isBotAdmin($channel, $bot_username, $bot_token) {
    $url = "https://api.telegram.org/bot$bot_token/getChatAdministrators?chat_id=@$channel";
    $res = @json_decode(file_get_contents($url), true);
    if (!$res || !$res['ok']) return false;
    foreach ($res['result'] as $admin) {
        if (isset($admin['user']['username']) && $admin['user']['username'] == $bot_username) return true;
    }
    return false;
}

// Remove invalid channels where bot is not admin anymore
$validChannels = [];
foreach ($channels as $channel) {
    if (isBotAdmin($channel, $bot_username, $bot_token)) {
        $validChannels[] = $channel;
    } else {
        // Alert user and admin
        $text = "
<b>❌ @$bot_username Bot Is Not Admin In Channel. 

🖇️Link : @$channel 

😕 Channel Auto-Deleted For User $number -> $fullname</b>";
        @file_get_contents("https://api.telegram.org/bot$bot_token/sendMessage?chat_id={$user['chat_id']}&text=" . urlencode($text). "&parse_mode=HTML");
        @file_get_contents("https://api.telegram.org/bot$bot_token/sendMessage?chat_id={$admin_chat_id}&text=" . urlencode($text). "&parse_mode=HTML");
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['channel'])) {
    $channelUrl = trim($_POST['channel']);
    $username = basename($channelUrl);

    if (isBotAdmin($username, $bot_username, $bot_token)) {
        if (!in_array($username, $channels)) {
            $channels[] = $username;
            file_put_contents($dataFile, json_encode($channels, JSON_PRETTY_PRINT));
            $msg = "

<b>✅ Channel Successfully Added Done 

🖇 Link : @$username

❤️ By User : $number -> $fullname</b>";
            @file_get_contents("https://api.telegram.org/bot$bot_token/sendMessage?chat_id=$admin_chat_id&text=" . urlencode($msg). "&parse_mode=HTML");
            @file_get_contents("https://api.telegram.org/bot$bot_token/sendMessage?chat_id={$user['chat_id']}&text=" . urlencode("<b>✅ Channel Successfully Added Done 

🖇 Link : @$username

❤️ By User : $number -> $fullname</b>"). "&parse_mode=HTML");
        }
    } else {
        @file_get_contents("https://api.telegram.org/bot$bot_token/sendMessage?chat_id={$user['chat_id']}&text=" . urlencode(" Please make @$bot_username admin in @$username before adding."). "&parse_mode=HTML");
    }
    header("Location: channel-data.php");
    exit();
}
...
if (isset($_GET['remove'])) {
    $remove = $_GET['remove'];
    $channels = array_filter($channels, fn($ch) => $ch != $remove);
    file_put_contents($dataFile, json_encode(array_values($channels), JSON_PRETTY_PRINT));
    
    // Prepare alert message
    $msg = "<b>❌ Channel Removed

🗑️ Removed : @$remove

👤 By User : $number -> $fullname</b>";

    // Send to admin
    @file_get_contents("https://api.telegram.org/bot$bot_token/sendMessage?chat_id=$admin_chat_id&text=" . urlencode($msg) . "&parse_mode=HTML");

    // Send to user
    @file_get_contents("https://api.telegram.org/bot$bot_token/sendMessage?chat_id={$user['chat_id']}&text=" . urlencode($msg) . "&parse_mode=HTML");
...
<style>
  ...
  <div class="card">
    <!-- Heading with Add Channel Button -->
<!-- Centered Manage Channels Button -->
<div style="display: flex; justify-content: center; align-items: center; margin: 20px 0; font-family: 'Poppins', sans-serif;">
  <button 
    ...
    <span class="material-icons" style="font-size: 20px;">add_circle</span> Manage Channels
  </button>
</div>
    <p style="font-weight: bold; color: gray; font-size: 14px;">There is no limit to adding channels.</p>
    <p style="margin: 10px 0; font-weight: bold; color: #007bff;">Total Added Channels: <?= count($channels) ?></p>
...
<div style="display: flex; gap: 12px; align-items: center; font-family: 'Poppins', sans-serif; margin: 20px 0;">
  <div style="position: relative; flex: 1;">
    ...
    <input 
      type="text" 
      name="channel" 
      placeholder="https://t.me/YourChannelUsername" 
      required
      style="font-weight: bold; width: 100%; padding: 12px 12px 12px 40px; border: 1.5px solid #d0d0d0; border-radius: 10px; font-size: 15px; transition: 0.3s; outline: none;"
...
<?php foreach ($channels as $ch): ?>
  <div style="font-weight: bold; background: #f9f9f9; border-left: 4px solid #007bff; padding: 12px 14px; border-radius: 10px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; font-family: 'Poppins', sans-serif; box-shadow: 0 2px 6px rgba(0,0,0,0.05);">
    <span style="font-weight: bold; font-weight: 500; font-size: 15px; font-weight: bold;">@<?= htmlspecialchars($ch) ?></span>
    <a href="?remove=<?= $ch ?>" style="background: linear-gradient(135deg, #ff416c, #ff4b2b); color: white; padding: 6px 12px; border-radius: 8px; text-decoration: none; display: flex; align-items: center; gap: 4px; font-size: 14px;">
      <span class="material-icons" style="font-weight: bold; font-size:18px;">delete</span> Remove
    </a>
  </div>
<?php endforeach; ?>

<!-- Bot Admin Info -->
<div style="font-weight: bold; margin-top: 24px; background: #e7f1ff; padding: 16px 14px; border-left: 5px solid #007bff; border-radius: 10px; font-family: 'Poppins', sans-serif; font-size: 15px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
  🤖 Make <a href="https://t.me/<?= $bot_username ?>" style="font-weight: bold; color: green;">@<?= $bot_username ?></a> an admin in your channel to enable advanced Lifafa features.
</div>

<!-- Bot Username with Read-only Input + Button Combo -->
<div style="display: flex; gap: 12px; align-items: center; font-family: 'Poppins', sans-serif; margin: 20px 0;">
  <div style="position: relative; flex: 1;">
    <span class="material-icons" style="position: absolute; top: 50%; left: 12px; transform: translateY(-50%); color: #888;">
      link
  ...
  <button 
    onclick="navigator.clipboard.writeText(document.getElementById('botUsername').value)" 
    style="background: linear-gradient(135deg, #00c6ff, #0072ff); color: white; padding: 12px 18px; border: none; border-radius: 10px; font-size: 14px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; box-shadow: 0 4px 10px rgba(0,0,0,0.1); transition: 0.2s;"
    onmouseover="this.style.background='linear-gradient(135deg, #0072ff, #0051d4)';"
    onmouseout="this.style.background='linear-gradient(135deg, #00c6ff, #0072ff)';"
  >
    <span class="material-icons">link</span> Copy
  </button>
</div>

<p style="font-weight: bold; margin-top: 10px; font-size: 13px; color: gray; text-align: center; font-family: 'Poppins', sans-serif;">
  👆 Tap to copy and share the bot username.
</p>
