...
if (!file_exists($userFile)) die("User data not found.");

$userData = json_decode(file_get_contents($userFile), true);
$config = json_decode(file_get_contents("adminrole/Admin/Admin.json"), true);
$botToken = $config['bot_token'];
$adminID = $config['admin_chat_id'];
...
// 💠 1. Update Profile Info (name, username, email)
if (isset($_POST['update_profile'])) {
    ...
    if (!password_verify($current_password, $userData['password'])) {
        ...
        if ($username !== $userData['username']) {
            $updateDetails[] = "👤 <b>Username:</b> <code>{$userData['username']}</code> ➜ <code>$username</code>";
            $userData['username'] = $username;
            $updates[] = "Username";
        }

        if ($name !== $userData['name']) {
            $updateDetails[] = "📝 <b>Name:</b> <code>{$userData['name']}</code> ➜ <code>$name</code>";
            $userData['name'] = $name;
            $updates[] = "Name";
        }

        if ($email !== $userData['email']) {
            $updateDetails[] = "📧 <b>Email:</b> <code>{$userData['email']}</code> ➜ <code>$email</code>";
        ...
        if (!empty($updates)) {
            ...
            $msg = "✅ <b>User Updated Profile Info</b>\n\n";
            $msg .= "<b>📱 Number:</b> <code>$number</code>\n" . implode("\n", $updateDetails);
            sendTelegram($botToken, $adminID, $msg);
...
if (isset($_POST['update_password'])) {
    ...
    if (!password_verify($current_password, $userData['password'])) {
        ...
        sendTelegram($botToken, $adminID, "🔒 <b>User Changed Password</b>\n\n📱 <b>Number:</b> <code>$number</code>");
...
if (isset($_POST['delete_account'])) {
    ...
    if (!password_verify($current_password, $userData['password'])) {
        $alert = "❌ Incorrect current password for account deletion.";
    } else {
        $userDir = "users/$number/";

        // Delete the entire folder and all files inside
        if (deleteDirectory($userDir)) {
            session_destroy();

            // Send Telegram notification
            $message = "⚠️ <b>User Deleted Account</b>\n\n📱 <b>Number:</b> <code>$number</code>";
            sendTelegram($botToken, $adminID, $message);

            // Redirect to goodbye page
            header("Location: goodbye.php");
            exit();
        } else {
            $alert = "⚠️ Failed to delete account directory.";
        }
    }
}
?>
...
<!--div class="card">
    ...
    <h2><span class="material-icons">account_circle</span> <b style="color: green;"><?= htmlspecialchars($number) ?></b></h2><br>
    </div>
...
<div class="card">
    ...
    <div class="auth-tabs">
        <div id="tab-pro" class="active" onclick="switchTab('pro')">Profile</div>
        <div id="tab-pass" onclick="switchTab('pass')">Password</div>
        <!--div id="tab-logo" onclick="switchTab('logo')">Logo</div-->
        <div id="tab-delete" onclick="switchTab('delete')">Delete</div>
    </div>
    <!-- ✅ Add Funds Tab -->
    <div id="form-pro" class="auth-form active">
<br>
        <?php if (!empty($alert)): ?>
            <div class="p-3 mb-4 rounded text-white <?= strpos($alert, '✅') === 0 ? 'bg-green-500' : 'bg-red-500' ?>">
                <?= $alert ?>
            </div>
        <?php endif; ?>
...
<svg class="icon11 cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24">
  <defs>
    <linearGradient id="gradPerson1" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" style="stop-color:#4facfe;stop-opacity:1" />
      <stop offset="100%" style="stop-color:#00f2fe;stop-opacity:1" />
    </linearGradient>
  </defs>
  <path fill="url(#gradPerson1)" d="M12 12c2.7 0 4.9-2.2 4.9-4.9S14.7 2.2 12 2.2 7.1 4.4 7.1 7.1 9.3 12 12 12zm0 2.2c-3.2 0-9.6 1.6-9.6 4.9v2.2h19.2v-2.2c0-3.3-6.4-4.9-9.6-4.9z"/>
</svg>
...
<svg class="icon11 cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24">
  <defs>
    <linearGradient id="gradAtSymbol" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" style="stop-color:#4facfe;stop-opacity:1" />
      <stop offset="100%" style="stop-color:#00f2fe;stop-opacity:1" />
    </linearGradient>
  </defs>
  <path fill="url(#gradAtSymbol)" d="M12 2C6.48 2 2 6.26 2 12s4.48 10 10 10c4.15 0 7.75-2.53 9.27-6.27.21-.5-.02-1.09-.53-1.31-.51-.22-1.09.02-1.31.53C18.04 17.71 15.19 20 12 20c-4.41 0-8-3.59-8-8s3.59-8 8-8c3.27 0 6.01 2.01 7.18 4.85.21.5.8.75 1.3.55.5-.2.75-.79.55-1.3C19.56 4.46 15.97 2 12 2zm0 6c-1.66 0-3 1.34-3 3s1.34 3 3 3c.68 0 1.3-.24 1.79-.64v.14c0 1.1-.9 2-2 2-.55 0-1 .45-1 1s.45 1 1 1c2.21 0 4-1.79 4-4V11c0-.55-.45-1-1-1s-1 .45-1 1v.5c-.28.3-.68.5-1.11.5-.83 0-1.5-.67-1.5-1.5S11.17 8 12 8s1.5.67 1.5 1.5c0 .28.22.5.5.5s.5-.22.5-.5c0-1.66-1.34-3-3-3z"/>
</svg>
...
<svg class="icon11 cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24">
  ...
  <path fill="url(#gradEmail)" d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 2v.01L12 13 4 6.01V6h16zM4 18V8l8 5 8-5v10H4z"/>
</svg>
...
<div class="group11">
    ...
    <input 
      ...
      required
    >     
    <svg id="toggleCurrent" class="icon11 cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24">
        <defs>
          <linearGradient id="gradEye1" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" style="stop-color:#4facfe;stop-opacity:1" />
            <stop offset="100%" style="stop-color:#00f2fe;stop-opacity:1" />
          </linearGradient>
        </defs>
        <path id="eyeIcon1" fill="url(#gradEye1)"
...
<br>
    ...
    <!-- ✅ Password Tab -->
<div id="form-pass" class="auth-form">
    <br>
<form method="POST">
<div class="group11">
  ...
  <div class="group11">
      ...
      <input 
        ...
        required
      >
      <svg id="toggleNew" class="icon11 cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24">
        <defs>
          <linearGradient id="gradEye2" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" style="stop-color:#4facfe;stop-opacity:1" />
            <stop offset="100%" style="stop-color:#00f2fe;stop-opacity:1" />
          </linearGradient>
        </defs>
        <path id="eyeIcon2" fill="url(#gradEye2)"
          d="M12 5C7 5 2.73 8.11 1 12c1.73 3.89 6 7 11 7s9.27-3.11 11-7c-1.73-3.89-6-7-11-7zm0 12
          c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8a3 3 0 100 6 3 3 0 000-6z" />
      </svg>
  ...
  <div class="group11">
      ...
      <input 
        ...
        required
      >
      <svg id="toggleConfirm" class="icon11 cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24">
        <defs>
          <linearGradient id="gradEye3" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" style="stop-color:#4facfe;stop-opacity:1" />
            <stop offset="100%" style="stop-color:#00f2fe;stop-opacity:1" />
          </linearGradient>
        </defs>
        <path id="eyeIcon3" fill="url(#gradEye3)"
          d="M12 5C7 5 2.73 8.11 1 12c1.73 3.89 6 7 11 7s9.27-3.11 11-7c-1.73-3.89-6-7-11-7zm0 12
          c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8a3 3 0 100 6 3 3 0 000-6z" />
      </svg>
  ...
  <div class="flex justify-center items-center w-full">
    <button type="submit" name="update_password" class="cssbuttons-io-button">
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
  <path d="M12 20h9" />
  <path d="M16.5 3.5a2.121 2.121 0 1 1 3 3L7 19l-4 1 1-4 12.5-12.5z" />
</svg>
  ...
  <br>
    ...
    <!-- ✅ Delete Tab -->
<div id="form-delete" class="auth-form">
   <br>
<form method="POST">
    <input 
      type="hidden" 
...
<div class="group11">
    ...
    <svg id="toggleCurrent" class="icon11 cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24">
     ...
     <div class="flex justify-center items-center w-full">
    <button type="submit" name="delete_account" class="cssbuttons-io-button">
...
<svg width="24" height="24" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
  <defs>
    ...
    <linearGradient id="binGradient" x1="0" y1="0" x2="0" y2="1">
      ...
      <stop offset="100%" stop-color="#8A2EFF"/>
    </linearGradient>
  ...
  <!-- Trash Can Body -->
  <rect x="16" y="20" width="32" height="36" rx="8" fill="url(#binGradient)"/>

  <!-- Trash Can Lid -->
  <rect x="12" y="8" width="40" height="16" rx="6" fill="url(#lidGradient)"/>

  <!-- Trash Lines -->
  <rect x="24" y="28" width="4" height="20" rx="2" fill="#EEE"/>
  <rect x="30" y="28" width="4" height="20" rx="2" fill="#EEE"/>
  <rect x="36" y="28" width="4" height="20" rx="2" fill="#EEE"/>

  <!-- Red Circle with X -->
  <circle cx="48" cy="48" r="10" fill="url(#circleGradient)"/>
  <line x1="44" y1="44" x2="52" y2="52" stroke="white" stroke-width="2" stroke-linecap="round"/>
  <line x1="52" y1="44" x2="44" y2="52" stroke="white" stroke-width="2" stroke-linecap="round"/>
</svg>
      <span><b>Delete My Account</b></span>
    </button>
  </div>
  <br>
</form>
    ...
    <!-- ✅ Logo Tab  -->
<!--div id="form-logo" class="auth-form">
   <br>
    ...
    <form method="POST">
<div class="ripple-btn" style="display: flex; justify-content: center; align-items: center; height: 100%;">
  <?php if (!empty($userData['photo']) && file_exists($userData['photo'])): ?>
      <img src="<?= $userData['photo'] ?>" alt="Profile" class="ripple-btn">
  <?php endif; ?>
</div>
<div class="container">
  <div class="search-container">
    ...
    <input 
      ...
      required
    >
<svg viewBox="0 0 24 24" class="search__icon" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="gradRupee" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" style="stop-color:#4facfe;stop-opacity:1" />
      <stop offset="100%" style="stop-color:#00f2fe;stop-opacity:1" />
    </linearGradient>
  </defs>
  <path fill="url(#gradRupee)" d="M6,4h12v2h-4.18c0.41,0.58,0.7,1.26,0.82,2H18v2h-3.09c-0.26,1.27-1.04,2.36-2.1,3.05L15.5,17H18v2h-6l-2.5-4H6v-2h4.5c0.83,0,1.5-0.67,1.5-1.5S11.33,10,10.5,10H6V8h4c-0.83,0-1.5-0.67-1.5-1.5S9.17,5,10,5H6V4z"/>
</svg>
...
<div class="container">
  <div class="search-container">
    <input 
      ...
      required
    >     
    <svg id="toggleCurrent" class="search__icon cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24">
        <defs>
          <linearGradient id="gradEye1" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" style="stop-color:#4facfe;stop-opacity:1" />
            <stop offset="100%" style="stop-color:#00f2fe;stop-opacity:1" />
  ...
  <script>
    function setupToggle(toggleId, inputId, iconId) {
      ...
      toggle.addEventListener("click", () => {
        ...
        icon.setAttribute("d",
          visible
            ? "M1 12s4-7 11-7 11 7 11 7-4 7-11 7S1 12 1 12zm11 3a3 3 0 100-6 3 3 0 000 6z"
            : "M12 5c-7 0-11 7-11 7s4 7 11 7 11-7 11-7-4-7-11-7zm0 12c-2.761 0-5-2.239-5-5s2.239-5 5-5 5 2.239 5 5-2.239 5-5 5zm0-8a3 3 0 100 6 3 3 0 000-6z"
        );
      });
    }

    setupToggle("toggleCurrent", "current_password", "eyeIcon1");
    setupToggle("toggleNew", "new_password", "eyeIcon2");
    setupToggle("toggleConfirm", "confirm_password", "eyeIcon3");
  </script>
