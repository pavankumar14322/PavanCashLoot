...
@mkdir("lifafa", 0777, true);
@mkdir("$userDir/channels", 0777, true);

// File paths
$walletFile = "$userDir/{$number}.txt";
$channelFile = "$userDir/channels/{$number}.json";

// Create missing files
if (!file_exists($walletFile)) file_put_contents($walletFile, "0");
if (!file_exists($channelFile)) file_put_contents($channelFile, json_encode([], JSON_PRETTY_PRINT));

// Load data
$balance = floatval(file_get_contents($walletFile));
$channels = json_decode(file_get_contents($channelFile), true) ?? [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $totalUsers = max(1, (int)$_POST['total_users']);
    $perUser = max(0.01, (float)$_POST['per_user']);
    $comment = htmlspecialchars(trim($_POST['comment']));
    $redirect = htmlspecialchars(trim($_POST['redirect']));
    $accessCode = htmlspecialchars(trim($_POST['access_code']));
    $requireChannel = isset($_POST['require_channel']);

    $totalAmount = $totalUsers * $perUser;

    if ($balance < $totalAmount) {
        $success = "<div class='bg-red-100 text-red-700 p-3 rounded mb-4'>❌ Insufficient Wallet Balance!</div>";
    } else {
        // Deduct balance
        $balance -= $totalAmount;
        ...
        // Generate ID
        $lifafaId = substr(md5(uniqid()), 0, 8);
        
        @mkdir("lifafa/$lifafaId", 0777, true);
        
        $lifafaFile = "lifafa/$lifafaId/{$lifafaId}.json";
        ...
        file_put_contents($lifafaFile, json_encode($lifafas, JSON_PRETTY_PRINT));
        
$adminData = json_decode(file_get_contents("adminrole/Admin/Admin.json"), true);
$admin_chat_id = $adminData['admin_chat_id'] ?? null;
$bot_token = $adminData['bot_token'] ?? null;


// Load user chat ID
$userData = json_decode(file_get_contents("$userDir/{$number}.json"), true);
$user_chat_id = $userData['chat_id'];  // Must be set in user JSON
$fullname = $userData['name'] ?? 'Unknown';

/*/ Create Telegram message
$telegramMsg = "<b>🎉 New Lifafa Created!\n\n"
    . "🆔 Lifafa ID: <code>$lifafaId</code>\n\n"
    . "🎁 Title: $title\n\n"
    . "👥 Total Users: $totalUsers\n\n"
    . "💸 Per User: ₹$perUser\n\n"
    . "💰 Total: ₹$totalAmount\n\n"
    . "🔐 Code: <code>$accessCode</code>\n\n"
    . "🩵 Comment: $comment\n\n"
    . "🖇️ Redirect: $redirect\n\n"
    . "🪴 Request Channels: $requireChannel\n\n"
    . "📍 Added Channels: $channels\n\n"
    . "🙋 By: $number ($fullname)\n\n"
    . "🔗 Link: <code>https://pavancashloot.xyz/Loots/lifafaw.php?id=$lifafaId</code></b>";*/
    
// Convert require_channel (true/false → Yes/No)
$requireChannelText = $requireChannel ? "Yes ✅" : "No ❌";

// Convert channels array → comma separated string with @
$channelsText = !empty($channels) 
    ? "@" . implode(", @", $channels) 
    : "None";

// Create Telegram message
$telegramMsg = "<b>⭐ New Lifafa Created By  ➪ $fullname ⭐\n\n"
    . "⭐ Lifafa ID ➪ <code>$lifafaId</code>\n\n"
    . "⭐ Title ➪ $title\n\n"
    . "⭐ Total Users ➪ $totalUsers\n\n"
    . "⭐ Per User ➪ ₹$perUser\n\n"
    ...
    . "⭐ Comment ➪ $comment\n\n"
    . "⭐ Redirect ➪ $redirect\n\n"
    . "⭐ Request Channels ➪ $requireChannelText\n\n"
    . "⭐ Added Channels ➪ $channelsText\n\n"
    . "⭐ By ➪ $number ($fullname)\n\n"
    . "⭐ Link ➪ <code>https://pavancashloot.xyz/Loots/lifafaw.php?id=$lifafaId</code></b>";
// Inline button JSON
$button = [
    'inline_keyboard' => [
        [
            ['text' => '🎁 Lifafa', 'url' => "https://pavancashloot.xyz/Loots/lifafaw.php?id=$lifafaId"]
        ,
...
// Send to admin
@file_get_contents("https://api.telegram.org/bot$bot_token/sendMessage?chat_id=$admin_chat_id&text=" . urlencode($telegramMsg) . "&parse_mode=HTML&reply_markup=$replyMarkup");

// Send to user (fixed this line)
@file_get_contents("https://api.telegram.org/bot$bot_token/sendMessage?chat_id=$user_chat_id&text=" . urlencode($telegramMsg) . "&parse_mode=HTML&reply_markup=$replyMarkup");
        $success = "
<div id='lifafa_id' style='display: flex; gap: 12px; align-items: center; font-family: Poppins, sans-serif; margin: 20px 0;'>
  <div style='position: relative; flex: 1;'>
    <span class='material-icons' style='position: absolute; top: 50%; left: 12px; transform: translateY(-50%); color: #888;'>link</span>
    <input 
      type='text' 
      value='https://pavancashloot.xyz/Loots/lifafaw.php?id=$lifafaId'
      readonly
...
<script>
function copyID() {
  const input = document.getElementById('lifafaLink');
  input.select();
...
<div class="flex justify-center items-center w-full gap-3 mb-6">
  ...
  <div class="group11">
    <input 
      ...
      required
    >
<svg class="icon11" width="24" height="24" viewBox="0 0 120 60" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="upiGradient" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" style="stop-color:#f26a21;stop-opacity:1" />
      <stop offset="100%" style="stop-color:#159b3f;stop-opacity:1" />
    </linearGradient>
  </defs>
  <text x="5" y="40" font-family="Arial, sans-serif" font-weight="bold" font-size="32" fill="url(#upiGradient)" transform="skewX(-12)">UPI</text>
  <polygon points="95,25 115,30 95,35" fill="#159b3f" />
</svg>
  ...
  <!-- Access Code -->
   ...
   <div class="group11">
    ...
    <input 
      ...
      type="text" 
      name="access_code" 
      placeholder="Enter Access Code (required to claim)" 
      required
    >
<svg class="icon11" width="24" height="24" viewBox="0 0 120 60" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
<!-- Require Channel Toggle (Advanced Style) -->
<div class="group11">
  <label class="toggle-container">
    ...
    <span class="toggle-label">Channels Request/ Adding Channel</span>
  </label>
...
<button 
  ...
  <span><b>Create</b></span>
    ...
    <span class="pricing-bottom-right ">
      <span onclick="mylifafa()"class="font-bold" style="color: white; font-size:17px;">My Lifafa's <small></small>
      </span>
    </span>
    </div>
...
<!--script>
function copyID() {
    const input = document.querySelector('#lifafa_id input');
    input.select();
    input.setSelectionRange(0, 99999); // for mobile devices

    navigator.clipboard.writeText(input.value).then(() => {
        showCopyToast("📋 Lifafa ID Copied!");
    }).catch(err => {
        showCopyToast("❌ Failed to copy ID");
        console.error(err);
    });
}
...
function copyID() {
  ...
  navigator.clipboard.writeText(input.value)
    ...
    .catch(() => {
      ...
      toast.querySelector(".message-text").textContent = "❌ Failed to copy!";
      toast.querySelector(".sub-text").textContent = "Try again later";
      showToast();
    });
}
