...
$balanceFile = "users/$number/$number.txt";
$userTxnFile = "users/$number/TXN/cashout/$number/$number.json";
$adminRequestFile = "adminrole/Admin/Rq-Cashout/$number/$number.json";
$userDataFile = "users/$number/$number.json";
$adminDataFile = "adminrole/Admin/Admin.json";

// Create directories if needed
@mkdir("users/$number/TXN/cashout/$number", 0777, true);
@mkdir("adminrole/Admin/Rq-Cashout/$number", 0777, true);

// Create files if missing
if (!file_exists($balanceFile)) file_put_contents($balanceFile, "0");
if (!file_exists($userTxnFile)) file_put_contents($userTxnFile, json_encode([]));
if (!file_exists($adminRequestFile)) file_put_contents($adminRequestFile, json_encode([]));

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $amount = floatval($_POST['amount']);
    $upi = trim($_POST['upi']);
    $balance = floatval(file_get_contents($balanceFile));

    // 🧾 Set limits
    $minLimit = 30;      // Minimum withdrawal ₹10
    $maxLimit = 10000;   // Maximum withdrawal ₹10,000

    if ($amount <= 0) {
        $msg = "Enter a valid amount.";
    } elseif ($amount < $minLimit) {
        $msg = "Minimum withdrawal is ₹$minLimit.";
    } elseif ($amount > $maxLimit) {
        $msg = "Maximum withdrawal is ₹$maxLimit.";
    } elseif ($amount > $balance) {
        $msg = "Insufficient balance.";
    } elseif (!preg_match("/^[\w.-]+@[\w.-]+$/", $upi)) {
        $msg = "Invalid UPI ID format.";
    } else {
        $txnId = strtoupper(uniqid("WD"));
        ...
        $userHistory = json_decode(file_get_contents($userTxnFile), true);
        $userHistory[] = $txnEntry;
        file_put_contents($userTxnFile, json_encode($userHistory, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));

        // Save to admin request
        $adminHistory = json_decode(file_get_contents($adminRequestFile), true);
        $adminHistory[] = [
            "txnid" => $txnId,
            "number" => $number,
            "name" => $name,
        ...
        file_put_contents($adminRequestFile, json_encode($adminHistory, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
        ...
        $adminChatId = $adminData['admin_chat_id'] ?? '';
        ...
        if ($botToken && $userChatId) {
            $userMsg = "✅ *Withdraw Request Submitted*\n\n*🆔 Txn ID:* `$txnId`\n*💰 Amount: ₹*`$amount`\n*📤 UPI: *`$upi`\n*⏰ Time:* $time\n\n*⏳ Status: Pending Admin Approval*";
            file_get_contents("https://api.telegram.org/bot$botToken/sendMessage?" . http_build_query([
                'chat_id' => $userChatId,
                'text' => $userMsg,
                'parse_mode' => 'Markdown'
        ...
        $msg = "Withdraw request sent to Admin.";
    }
}
?>
...
<style>
        .msg { margin-top: 20px; background: #e2ffe2; padding: 10px; border-left: 5px solid #4CAF50; border-radius: 4px; }
...
<div class="card">
    ...
    <div style="padding: 5px; background-color: #fff; text-align: center;">
     <div class="statuss" style="display: flex;">
        ...
        <b>Withdrawal your wallet securely and 24 Hours </b>
        
    </div>
        

<div class="change-id statuss" style="display: flex; align-items: center; gap: 8px; text-align: center;">
    
  <b style="color: #FF0000;"> Minimum > 30₹ </b> <b style="color: #FEBE10;"> Maximum > 100₹</b>
...
<div class="change-id statuss" style="display: flex; align-items: center; gap: 8px;">
  ...
  <b style="color: #FF0000;"> </b> <b style="color: #FEBE10;">

        <div class="msg"><?= $msg ?></div> </b>
</div>
    <?php endif; ?>
...
<div class="group11">
    ...
    >
    <!-- Gradient ₹ Icon -->
<svg viewBox="0 0 24 24" class="icon11" xmlns="http://www.w3.org/2000/svg">
  <defs>
