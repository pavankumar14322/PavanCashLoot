    ...
    if ($_FILES['photo']['error'] === 0) {
        if ($_FILES['photo']['size'] <= 5 * 1024 * 1024) {
            ...
            if (in_array($ext, $allowedExts)) {
                ...
                $unicnumbers = str_pad(rand(0, 999), 3, '0', STR_PAD_LEFT);

                // Construct new file name
                $photoPath = "$photoDir$number.$unicnumbers.$ext";
...
<link href="https://unpkg.com/lucide@latest/umd/lucide.min.js" rel="preload" as="script">
...
<div class="card">
  ...
  <div class="profile-info">
    <h3 class="name">
      ...
      <input type="text" readonly value="<?= htmlspecialchars($userData['name']) ?>">
    </h3>
    <h3 class="contact">
      <i data-lucide="at-sign"></i>
      <input type="text" readonly value="@<?= htmlspecialchars($userData['username']) ?>">
    </h3>
    <p class="contact">
      <i data-lucide="mail"></i>
      <input type="text" readonly value="<?= htmlspecialchars($userData['email']) ?>">
    </p>
    <p class="contact">
      <i data-lucide="phone"></i>
      <input type="text" readonly value="<?= htmlspecialchars($userData['number']) ?>">
    </p>
    <hr class="divider">
  </div>
<script>
...
function previewImage(event) {
  ...
  if (file && file.type.startsWith('image/')) {
    ...
    reader.onload = function(e) {
      preview.innerHTML = `<img src="${e.target.result}" alt="Preview" style="max-width:150px;border-radius:12px;">`;
    };
    reader.readAsDataURL(file);
  } else {
    preview.innerHTML = "<p>Invalid file type</p>";
  }
}
</script>

<form method="POST" enctype="multipart/form-data" class="upload-form0">
  <div class="image-upload-container">
    <input type="file" id="imageInput" name="photo" accept="image/*" onchange="previewImage(event)" required>
    <!--label for="imageInput" class="image-upload">    <span class="material-icons">upload</span> Upload Image</label-->
    <label for="imageInput" class="image-upload" style="
    display: flex;
    justify-content: center;
    align-items: center;
    gap: 6px;
...
<!--svg width="26" height="26" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  <path fill="url(#uploadGradient)" d="M5 17a1 1 0 0 0 0 2h14a1 1 0 1 0 0-2H5zm7-12a1 1 0 0 0-1 1v8.59l-2.3-2.3a1 1 0 1 0-1.4 1.42l4 4a1 1 0 0 0 1.4 0l4-4a1 1 0 0 0-1.4-1.42L13 14.59V6a1 1 0 0 0-1-1z"/>
</svg-->
